...
### Fase 9.20 — Comparación rigurosa vs. ELSA Speak/Loora/BoldVoice + plan de mejora de 5 fases ✅ (A–D implementadas, E documentada)

El usuario señaló: "los contenidos que estamos poniendo en módulo 1 y 2 no me convencen, hacé una comparación vs Loora, Elsa Speak, etc. y hacé un plan de mejora... sé riguroso y estratégico." Investigado con web search real (no memoria/especulación) antes de proponer nada.

**Hallazgos de la investigación** (fuentes en el mensaje original de la comparación): ELSA Speak tiene 1,400–7,100+ lecciones, feedback fonema por fonema en tiempo real con espectrograma, y **personaliza por idioma nativo del alumno** (30 L1 disponibles) — anticipa errores específicos por L1, algo que SpeakWise no hacía a pesar de que toda la app asume un alumno hispanohablante. Loora no es drilling, es conversación con corrección en vivo + una herramienta de traducción (el alumno escribe en su idioma lo que quiere decir). BoldVoice explica mecánica articulatoria (lengua, mandíbula, aire), no solo marca ortográfica. **Diagnóstico clave:** los patrones de módulo 1 elegidos hasta ahora eran ad-hoc (lo que se me ocurrió o pediste durante la sesión) — datos correctos sobre inglés, pero no priorizados por impacto real para un hispanohablante específicamente (más "curiosidad ortográfica" que error frecuente).

**Plan de 5 fases, aprobado por el usuario en bloque ("aprouebo todo de A hasta E"):**

**Fase A — Re-curar módulo 1 con errores respaldados por análisis contrastivo español→inglés ✅** 5 patrones nuevos en `corpus/patterns.csv`: vocal ɪ/iː (ship/sheep — el español no distingue vocal corta/larga), v≠b (el español las pronuncia casi igual), θ/ð≠s/t/d (el sonido "th" no existe en español), consonante final que se cae (el español casi no tiene sílabas cerradas), epéntesis de e- antes de s+consonante (school→"eschool"). Se reprioriza el pool existente: `-age/-idge`, `sílabas elididas`, `kn-/wr-`, `t muda después de n` bajan a priority 2 (menor impacto real); `schwa` y `-tion/-sion` se mantienen en priority 1 (ya coinciden con lo que ELSA prioriza). De paso, se agregó `p.id ASC` como desempate final determinístico en `_pattern_of_the_day()` (antes dependía del orden implícito de SQLite, no garantizado) — cambió el pattern de cold-start de "-age/-idge" a "-tion/-sion", test actualizado.

**Fase B — Enlace entre palabras (connected speech cross-word) ✅** Antes solo cubríamos fenómenos DENTRO de una palabra (ej. la t de "internet"); el fenómeno más frecuente en habla real es el enlace ENTRE palabras ("get it"→"geddit", "want to"→"wanna"). 2 patrones nuevos con frases de 2 palabras como family. Nueva función `analyze_phrase_linking()` (`phoneme.py`) — **limitación documentada honestamente**: a diferencia de `analyze_stress`/`analyze_phonemes` (comparan fonemas reales vía el modelo acústico), acá no hay forma de verificar acústicamente si el alumno enlazó bien — Whisper transcribe texto ortográfico, no distingue "get it" enlazado de "get it" con pausa. Se verifica por presencia de la frase completa en la transcripción (mismo criterio que `log_chunk_used`) — "correct" significa "intentó la frase", no "la enlazó perfecto". Detección acústica real de flapping cross-word queda para Fase E.

**Fase C — Módulo 2: traducí lo que querés decir (estilo Loora) ✅** Nuevo `services/translate.py` + `POST /api/translate-practice`: el alumno escribe en español lo que quiere decir, el LLM devuelve la traducción natural (no literal) + una nota de uso si hace falta. Complementa (no reemplaza) los 30 modismos curados — vocabulario que nace de la necesidad real del alumno en el momento, no solo de una rotación fija.

**Fase D — Diagnóstico inicial que siembra la rotación con datos reales ✅** Nuevo `services/diagnostic.py`: `build_diagnostic_plan()` junta las palabras de todos los patrones priority=1 (los de mayor impacto, Fase A); `apply_diagnostic_results()` siembra `pattern_progress` de cada patrón con el accuracy real de esa única grabación (evidencia positiva únicamente — si ninguna palabra de un patrón aparece en la transcripción, ese patrón no se toca, mismo criterio que `user_progress`). Nuevo botón "🎯 Take diagnostic" en módulo 1 (disponible siempre, no solo una vez). **Verificado en vivo con ciclo TTS→diagnostic real:** las 7 palabras/patrones priority=1 quedaron sembrados con accuracy real en una sola grabación (0.0 a 0.8 según el patrón, reflejando que el modelo de fonemas efectivamente distingue estos sonidos).

**Limitación real encontrada en vivo, no resuelta:** `_pattern_of_the_day()` ordena por `accuracy ASC, sessions_practiced ASC` — un patrón recién diagnosticado con accuracy=0.0 y `sessions_practiced=1` puede perder contra un patrón *nunca tocado* (`sessions_practiced=0`, mismo accuracy por defecto), porque el desempate favorece "menos practicado" primero. Confirmado en vivo: después del diagnóstico, `/api/today` sirvió `-age/-idge` (priority 2, nunca practicado) en vez de "vocal corta i vs. larga i" (priority 1, recién diagnosticado con 0% de accuracy real). Arreglarlo bien requiere un scheduler de 2 fases (agotar primero los patrones nunca intentados, después ordenar por accuracy real) — cambiar la fórmula actual con un parche simple arriesgaba romper el fix anti-repetición ya probado de Fase 9.3 (`test_pattern_of_the_day_prefers_least_practiced`). Documentado acá en vez de arriesgar una regresión de último momento; el diagnóstico sigue siendo útil (siembra datos reales que van ganando peso a medida que el resto de los patrones también se practican), solo no domina la selección INMEDIATAMENTE después de correrlo.

**Fase E — Feedback fonema por fonema con score visual (aspiracional, documentado, no implementado) ✅**

Alcance de lo que tiene ELSA hoy y SpeakWise no: puntaje por cada fonema producido (no solo el de acento primario), visualizado sobre un espectrograma, en tiempo real mientras el alumno habla.

Lo que haría falta construir, en orden de dependencia:
1. **Alineación fonémica completa** — hoy `phoneme.py` solo compara el fonema con acento primario (`expected_focus_phoneme`) contra si aparece en la salida de wav2vec2 para la ventana de tiempo de la palabra completa. Haría falta alinear CADA fonema esperado (según CMU dict) contra un segmento de tiempo específico dentro de la palabra — técnica: forced alignment (ej. Montreal Forced Aligner, o CTC-segmentation sobre la salida del mismo modelo wav2vec2 ya en uso).
2. **Score de Goodness of Pronunciation (GOP) por fonema** — no un simple correcto/incorrecto binario como ahora, sino una probabilidad/distancia acústica por fonema contra una referencia (técnica estándar en la industria, la que usan ELSA/BoldVoice/Speechace).
3. **UI de espectrograma con color por fonema** — trabajo de frontend nuevo (canvas, no components existentes), verde/amarillo/rojo por segmento.
4. **Tiempo real vs. post-grabación** — ELSA puntúa mientras hablás; el pipeline actual es grabar→enviar→analizar (mismo patrón que todo el resto de la app). Streaming real sería un cambio de arquitectura más grande, probablemente fuera de alcance incluso para esta fase.

Estimación honesta: esto es trabajo de varias semanas de investigación + ingeniería, no un incremento chico — amerita su propia iteración (`BACKLOG.md`, ítem agregado a "Backlog futuro"), no un agregado apurado a ITER-2.

Tests nuevos: `test_phonetic_patterns_exactly_13` (Fase A) — `test_seed.py`; `test_pattern_of_the_day_includes_stress_caps_and_respelling` actualizado (nuevo cold-start winner) — `test_curriculum.py`; `test_analyze_phrase_linking_*` (Fase B) — `test_phoneme.py`; `test_transcribe_and_analyze_merges_phrase_linking_into_stress_results` — `test_acoustic.py`; `test_get_translation_practice_*` (Fase C, nuevo) — `test_translate.py`; `test_translate_practice_*` — `test_session.py`; `test_index_html_has_translate_practice_elements` + `test_app_js_translate_practice_*` — `test_session_modules.py`; `test_build_diagnostic_plan_*` + `test_apply_diagnostic_results_*` (Fase D, nuevo) — `test_diagnostic.py`; `test_diagnostic_words_*` + `test_diagnostic_seeds_pattern_progress_from_real_audio` — `test_session.py`; `test_index_html_has_diagnostic_elements_in_module_1` + `test_app_js_diagnostic_*` — `test_session_modules.py`.

**Verificado en vivo:** Fase A/B — patrón "enlace entre palabras" en pantalla con markup y ciclo TTS→transcribe→analyze completo (`get it`/`turn on`/etc. todos `correct: true`). Fase C — traducción real de "Voy a llegar un poco tarde" → "I'm going to be a bit late." con nota de uso, vía LLM real. Fase D — diagnóstico real con las 35 palabras de los 7 patrones priority=1, sembrando `pattern_progress` con accuracy real por patrón en una sola grabación TTS.

Suite completa: 271/271 (3 de integración deseleccionados).
